...
record DispatcherDelegate(ImperativeDispatcher delegate, Context context, ExecutorService executor) implements Dispatcher {
    ...
    public Uni<Dispatcher> subscribe(String subject) {
        return Uni.createFrom().item(Unchecked.supplier(() -> delegate.subscribe(subject)))
                .onItem().ifNotNull()
                .transform(dispatcher -> Dispatcher.of(ImperativeDispatcher.of(dispatcher), context, executor))
                .emitOn(context::runOnContext);
    }
    ...
    public Uni<Subscription> subscribe(String subject, MessageHandler handler) {
        return Uni.createFrom()
                .item(Unchecked.supplier(() -> delegate.subscribe(subject,
                        msg -> handler.onMessage(Message.of(NativeMessage.of(msg), context)))))
                .onItem().ifNotNull()
    ...
    public Uni<Subscription> subscribe(String subject, String queue, MessageHandler handler) {
        return Uni.createFrom()
                .item(Unchecked.supplier(() -> delegate.subscribe(subject, queue,
                        msg -> handler.onMessage(Message.of(NativeMessage.of(msg), context)))))
                .onItem().ifNotNull()
                .transform(subscription -> Subscription.of(ImperativeSubscription.of(subscription), context, executor))
                .emitOn(context::runOnContext);
