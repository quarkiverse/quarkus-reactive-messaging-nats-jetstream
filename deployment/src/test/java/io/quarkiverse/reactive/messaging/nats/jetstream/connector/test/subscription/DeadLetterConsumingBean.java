package io.quarkiverse.reactive.messaging.nats.jetstream.connector.test.subscription;
...
public class DeadLetterConsumingBean {
    ...
    public DeadLetterConsumingBean(Client client, Serializer serializer) {
        this.client = client;
    ...
    public Uni<Void> durableConsumer(Message<Data> message) {
        return Uni.createFrom().item(message)
                .onItem()
                .invoke(() -> logger.infof("Received message on unstable-data-consumer channel: %s", message.getPayload()))
                .onItem()
    ...
    public Uni<Void> deadLetter(Message<Advisory> message) {
        ...
        return client.streamManagement().message(advisory.stream(), advisory.stream_seq())
                .onItem()
                .invoke(dataMessage -> lastData.set(serializer.readValue(
                        dataMessage.payload().orElseThrow(() -> new RuntimeException("Failed to deserialize data")),
                        Data.class)))
