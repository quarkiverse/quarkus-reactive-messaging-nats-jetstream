...
public class FetchMessagesTest implements MessageConsumer<Object> {
    ...
    static QuarkusExtensionTest runner = new QuarkusExtensionTest()
            .setArchiveProducer(
                    () -> ShrinkWrap.create(JavaArchive.class)
                            .addClasses(TestSpanExporter.class, Data.class, ConsumerConfiguration.class, MessageConsumer.class))
    ...
    void fetchOneMessage() {
        ...
        final var received = client.next("fetch-test", "fetch-data-consumer", TIMEOUT).await().atMost(TIMEOUT);
    ...
    void fetchTwoMessages() {
        ...
        final var received1 = client.next("fetch-test", "fetch-data-consumer", TIMEOUT).await().atMost(TIMEOUT);
    ...
    void fetchOneNotAcknowledgedWithDelayMessage() {
        ...
        assertThat(received1).isNotNull();
        assertThat(received1.getPayload()).isEqualTo(data1);
        notAcknowledge(received1, new RuntimeException(), Metadata.of(NotAcknowledgeMetadata.of(TIMEOUT.plusSeconds(5))))
                .await().atMost(TIMEOUT);

        // The message should not be available immediately after a nack with delay
        assertThat(client.next("fetch-test", "fetch-data-consumer", TIMEOUT).await().atMost(TIMEOUT.plusSeconds(1))).isNull();

        // Attempt to fetch the message until it is available again
        final var received2 = client.next("fetch-test", "fetch-data-consumer", Duration.ofSeconds(35L)).await()
                .atMost(Duration.ofSeconds(35L));
        assertThat(received2).isNotNull();
    ...
    void subjectTokens() {
        ...
        final var configuration1 = new ConsumerConfiguration(data1.resourceId(),
                "resources." + data1.resourceId());
        final var configuration2 = new ConsumerConfiguration(data2.resourceId(),
                "resources." + data2.resourceId());
        final var configuration3 = new ConsumerConfiguration(data3.resourceId(),
        ...
        client.consumerManagement("fetch-test").addIfAbsent(configuration4).await().atMost(TIMEOUT);

        client.publish(Message.of(data1), "fetch-test", "resources." + data1.resourceId())
                .await().atMost(TIMEOUT);
        client.publish(Message.of(data2), "fetch-test", "resources." + data2.resourceId())
                .await().atMost(TIMEOUT);
        client.publish(Message.of(data3), "fetch-test", "resources." + data3.resourceId())
                .await().atMost(TIMEOUT);
        client.publish(Message.of(data4), "fetch-test", "resources." + data4.resourceId())
                .await().atMost(TIMEOUT);

        final var received1 = client.next("fetch-test", data1.resourceId(), TIMEOUT).await().atMost(TIMEOUT);
        acknowledge(received1).await().atMost(TIMEOUT);
        assertThat(received1.getPayload()).isEqualTo(data1);

        final var received2 = client.next("fetch-test", data2.resourceId(), TIMEOUT).await().atMost(TIMEOUT);
        acknowledge(received2).await().atMost(TIMEOUT);
        assertThat(received2.getPayload()).isEqualTo(data2);

        final var received3 = client.next("fetch-test", data3.resourceId(), TIMEOUT).await().atMost(TIMEOUT);
        acknowledge(received3).await().atMost(TIMEOUT);
        assertThat(received3.getPayload()).isEqualTo(data3);

        final var received4 = client.next("fetch-test", data4.resourceId(), TIMEOUT).await().atMost(TIMEOUT);
    ...
    void addAndRemoveSubject() {
        ...
        final var configuration1 = new ConsumerConfiguration(data1.resourceId(), data1.resourceId());
        final var configuration2 = new ConsumerConfiguration(data2.resourceId(), data2.resourceId());

        client.consumerManagement("fetch-test").addIfAbsent(configuration1).await().atMost(TIMEOUT);
        client.consumerManagement("fetch-test").addIfAbsent(configuration2).await().atMost(TIMEOUT);

        client.publish(Message.of(data1), "fetch-test", data1.resourceId()).await()
                .atMost(TIMEOUT);
        client.publish(Message.of(data2), "fetch-test", data2.resourceId()).await()
                .atMost(TIMEOUT);
        ...
        client.streamManagement().removeSubject("fetch-test", data1.resourceId()).await().atMost(TIMEOUT);
        client.streamManagement().removeSubject("fetch-test", data2.resourceId()).await().atMost(TIMEOUT);
    }
    ...
    void fetchMessages() {
        ...
        final var received = client.fetch("fetch-test", consumer, TIMEOUT, 2)
                .onItem()
                .transformToUniAndMerge(message -> acknowledge(message).onItem()
                        .transform(ignored -> message.getPayload()))
                .collect().asList()
                .await().atMost(TIMEOUT);
